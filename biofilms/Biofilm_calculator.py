...
thresh_frame = cv2.threshold(src=blurred, thresh=40, maxval=255, type=cv2.THRESH_BINARY)[1]
...
contours, _ = cv2.findContours(image=erosion_frame, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
...
# plt.imshow(cv2.cvtColor(erosion_frame, cv2.COLOR_BGR2RGB))
...
#%% 
        ...
        total_area = area_purple + area_non_purple
        ...
        hist = cv2.calcHist([trimmed_image], [0], None, [256], [0, 256])
        ...
        std_deviation = np.std(trimmed_image)/255
...
# if "PTFE" in file:
#     results.to_excel(file[7:18] + ".xlsx")
